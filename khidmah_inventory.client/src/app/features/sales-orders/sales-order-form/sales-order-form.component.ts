...
import { SalesOrder, CreateSalesOrderRequest, UpdateSalesOrderRequest } from '../../../core/models/sales-order.model';
...
export class SalesOrderFormComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.allowOversell = localStorage.getItem('sales_order_allow_oversell') === 'true';
    ...
    if (this.isEditMode && orderId) {
      ...
      this.subs.add(
        this.realtimeSync.watchEntity('SalesOrder', orderId).subscribe(() => {
          if (!this.saving && !this.loading) {
            this.showToastMessage('warning', 'This sales order was updated by another user. Data has been refreshed.');
            this.loadSalesOrder(orderId);
          }
        })
      );
  ...
  save(nextStatus?: string): void {
    ...
    if (this.isEditMode && this.salesOrder) {
      this.salesOrderApiService.updateSalesOrder(this.salesOrder.id, request as UpdateSalesOrderRequest).subscribe({
        next: (response) => {
          if (response.success) {
            this.recordTimeline('Updated', `Order updated with state ${this.formData.workflowStatus}`);
            this.persistDraft();
            this.showToastMessage('success', 'Sales order updated successfully');
          } else {
            this.showToastMessage('error', response.message || 'Failed to update sales order');
          }
          this.saving = false;
        },
        error: (error) => {
          this.showToastMessage('error', this.extractServerError(error, 'Error updating sales order'));
          this.saving = false;
        }
      });
    } else {
      this.salesOrderApiService.createSalesOrder(request).subscribe({
        next: (response) => {
          if (response.success) {
        ...
        error: (error) => {
          this.showToastMessage('error', this.extractServerError(error, 'Error creating sales order'));
          this.saving = false;
        }
      });
    }
  ...
  onLineValueChange(line: SalesOrderLineVm): void {
    this.recalculateLine(line);
    this.queueAutosave();
  }

  addLine(copyFrom?: SalesOrderLineVm): void {
  ...
  onProductSelected(lineIndex: number, productId: string): void {
    ...
    const rememberedPrice = this.getRememberedPrice(product.id) ?? product.salePrice ?? 0;
  ...
  applyBulkPaste(): void {
    ...
    for (const row of lines) {
      ...
      parsed.push(this.recalculateLine({
        ...
        unitPrice: Number(priceRaw || product.salePrice || 0) || 0,
  ...
  cloneAsNew(): void {
    ...
    localStorage.setItem('sales_order_clone_payload', JSON.stringify(payload));
    this.router.navigate(['/sales-orders/new']);
  }

  convertToInvoice(): void {
    if (!this.salesOrder?.id) return;
    this.documentApiService.generateInvoicePdf(this.salesOrder.id).subscribe({
      next: (blob) => {
      ...
      error: () => {
        this.showToastMessage('error', 'Failed to generate invoice document');
      }
    });
  }
  ...
  private setupAutosave(): void {
    ...
    const clonePayload = localStorage.getItem('sales_order_clone_payload');
    if (!this.isEditMode && clonePayload) {
      try {
        ...
        this.lines = (parsed.lines || []).map((line: SalesOrderLineVm) => this.recalculateLine({ ...line, uid: this.generateLineUid() }));
      } catch {
        // ignore malformed clone payload
      }
      localStorage.removeItem('sales_order_clone_payload');
    }
  }
  ...
  private validateBusinessRules(): boolean {
    ...
    const hasNegativeMargin = this.lines.some(line => {
    ...
    if (hasNegativeMargin) {
      const proceed = window.confirm('Negative profit detected on one or more lines. Continue?');
      if (!proceed) return false;
    }

    if (!this.allowOversell) {
      const invalidStock = this.lines.find(line => line.availableStock != null && line.quantity > line.availableStock);
      if (invalidStock) {
        this.showToastMessage('error', `Line with SKU ${invalidStock.sku} exceeds available stock.`);
        return false;
      }
    }
  ...
  private createEmptyLine(): SalesOrderLineVm {
    return {
      uid: this.generateLineUid(),
      productId: '',
      productName: '',
  ...
  private recalculateLine(line: SalesOrderLineVm): SalesOrderLineVm {
  ...
  private restoreDraft(): void {
    ...
    try {
      ...
      const draftLines = (draft.lines || []) as SalesOrderLineVm[];
  ...
  private rememberRecentSelections(): void {
    ...
    localStorage.setItem('so_recent_prices', JSON.stringify(priceMap));
  }

  private getRememberedPrice(productId: string): number | null {
    try {
      const raw = localStorage.getItem('so_recent_prices');
      if (!raw) return null;
  ...
  private get draftKey(): string {
    return this.isEditMode && this.currentOrderId
      ? `sales_order_draft_${this.currentOrderId}`
      : 'sales_order_draft_new';
  }
}
